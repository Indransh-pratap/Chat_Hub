  ...
  return (
    <div className="h-full flex flex-col lg:grid lg:grid-cols-[240px_1fr_320px] bg-[#050505] overflow-hidden">
      ...
      <div className="flex flex-col h-[40vh] lg:h-full border-t lg:border-t-0 lg:border-l border-white/5 bg-black/60 backdrop-blur-md overflow-hidden">
        ...
        <form onSubmit={sendGameMessage} className="p-3 lg:p-4 bg-black/95 border-t border-white/5 flex gap-2">
            <button type="button" className="p-2 text-gray-500 hover:text-white transition-colors hidden sm:block"><Image className="w-4 h-4" /></button>
            <input 
                type="text" 
                value={chatInput}
                onChange={(e) => setChatInput(e.target.value)}
                placeholder="Secure transmission..." 
                className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-xs text-white placeholder:text-gray-600 focus:outline-none focus:border-red-500/50 transition-all shadow-inner"
            />
            <button type="submit" className="px-4 py-2 bg-red-600 rounded-xl text-white hover:bg-red-500 transition-all shadow-[0_0_15px_rgba(255,0,60,0.4)] active:scale-95">
